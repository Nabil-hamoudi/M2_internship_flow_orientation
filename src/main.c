...
void print_graph_details(struct graph *G, flotant v_res, flotant v_arc) {
    ...
    // Affichage des Sommets (inchangé)
    ...
    for (int i = 0; i < G->nb_arcs; i++) {
        ...
        flotant v_input = 0.0;
        if (a->diametre > 0.0) { // On n'affiche pas de vitesse pour les arcs virtuels sans diamètre
            v_input = (a->source->type == RESERVOIR) ? v_res : v_arc;
        }
        ...
        printf("%-5d | %-8s | %3d -> %-9d | %-8.1f | %-8.1f | %-12.4f | %-12.4f | %-10.2f\n",
               ...
               a->capacite,
               a->flow,
               v_input*60); // Affichage direct du paramètre
    }
    printf("=====================================================================================\n\n");
}
